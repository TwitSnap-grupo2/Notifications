...
router.post("/:id/devices", async (req, res, next) => {
  try {
    ...
    const updatedUser = await notificationsService.addUserDevice(userId, device);
...
router.post("/:id", async (req, res, next) => {
  try {
    ...
    const newNotification = await notificationsService.createNotification(userId, notificationData);
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const notifications = await notificationsService.getUserWithNotifications(userId);
...
router.patch("/:id", async (req, res, next) => {
  try {
    ...
    const modifiedNotification = await notificationsService.modifyNotification(notificationId, newStatus);
...
router.get("/:id/unseen", async (req, res, next) => {
  try {
    ...
    const unseenNotificationsCount = await notificationsService.getUnseenNotificationCount(userId);
